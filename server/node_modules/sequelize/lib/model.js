...
See https://sequelize.org/main/manual/model-basics.html#caveat-with-public-class-fields`);
  ...
  static init(attributes, options = {}) {
    ...
    if (this.options.timestamps) {
      for (const key of ["created_at", "updated_at", "deletedAt"]) {
      ...
      if (this.options.created_at !== false) {
        this._timestampAttributes.created_at = typeof this.options.created_at === "string" ? this.options.created_at : "created_at";
        this._readOnlyAttributes.add(this._timestampAttributes.created_at);
      }
      if (this.options.updated_at !== false) {
        this._timestampAttributes.updated_at = typeof this.options.updated_at === "string" ? this.options.updated_at : "updated_at";
        this._readOnlyAttributes.add(this._timestampAttributes.updated_at);
      }
  ...
  static async upsert(values, options) {
    ...
    if (created_atAttr && !insertValues[created_atAttr]) {
  ...
  static async bulkCreate(records, options = {}) {
    ...
    const recursiveBulkCreate = async (instances2, options2) => {
      ...
      const created_atAttr = model._timestampAttributes.created_at;
      const updated_atAttr = model._timestampAttributes.updated_at;
      if (options2.updateOnDuplicate !== void 0) {
        if (Array.isArray(options2.updateOnDuplicate) && options2.updateOnDuplicate.length) {
          options2.updateOnDuplicate = _.intersection(_.without(Object.keys(model.tableAttributes), created_atAttr), options2.updateOnDuplicate);
        } else {
      ...
      if (options2.individualHooks) {
        ...
        records = instances2.map((instance) => {
          ...
          if (created_atAttr && !values[created_atAttr]) {
            ...
            if (!options2.fields.includes(created_atAttr)) {
              options2.fields.push(created_atAttr);
            }
          }
          if (updated_atAttr && !values[updated_atAttr]) {
            values[updated_atAttr] = now;
            if (!options2.fields.includes(updated_atAttr)) {
              options2.fields.push(updated_atAttr);
            }
          }
  ...
  static async update(values, options) {
    ...
    if (options.fields && options.fields instanceof Array) {
      ...
      if (updated_atAttr && !options.fields.includes(updated_atAttr)) {
    ...
    } else if (_.isEmpty(valuesUse) || Object.keys(valuesUse).length === 1 && valuesUse[this._timestampAttributes.updated_at]) {
      result = [0];
    } else {
  ...
  static async increment(fields, options) {
    ...
    const updated_atAttr = this._timestampAttributes.updated_at;
    if (!options.silent && updated_atAttr && !incrementAmountsByField[updated_atAttr]) {
      const attrName = this.rawAttributes[updated_atAttr].field || updated_atAttr;
      extraAttributesToBeUpdated[attrName] = this._getDefaultTimestamp(updated_atAttr) || Utils.now(this.sequelize.options.dialect);
    }
    const tableName = this.getTableName(options);
    let affectedRows;
    if (isSubtraction) {
  ...
  async save(options) {
    ...
    if (updated_atAttr && options.fields.length > 0 && !options.fields.includes(updated_atAttr)) {
      options.fields.push(updated_atAttr);
    }
    ...
    if (this.isNewRecord === true) {
      if (created_atAttr && !options.fields.includes(created_atAttr)) {
    ...
    if (updated_atAttr && !options.silent && options.fields.includes(updated_atAttr)) {
