...
class AuthService extends GetxService {
  ...
  final Rx<ProfileModel?> currentUserProfile = Rx<ProfileModel?>(null);
  ...
  Future<Map<String, dynamic>?> googleLogin(
    ...
    try {
      ...
      if (response != null && response['success'] == true) {
        _storage.write('isLoggedIn', true);
        _storage.write('authData', response);
        _storage.write('jwtToken', response['token']);
      ...
      print('❌ Google login service error: $e');
      rethrow;
    }
  }
  ...
  Future<bool> sendOtpByEmail(String email) async {
    try {
      isLoading.value = true;
      final data = await _apiClient
          .post('/auth/send-otp-byemail', data: {'email': email});
      return data != null && data['success'] == true;
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> updateRole(JoinAsRole role) async {
    try {
      ...
      String roleValue;
      switch (role) {
        case JoinAsRole.guardian:
          roleValue = 'guardian';
          break;
        case JoinAsRole.individual:
