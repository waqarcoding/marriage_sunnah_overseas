...
const STEPS = [
    ...
    { id: 7, label: "Guardian", icon: Shield },
    { id: 8, label: "Preferences", icon: Heart },
]
            ...
            <div className="flex justify-between mt-2">
                {STEPS.map(s => (
                    <div key={s.id} className={`flex flex-col items-center gap-0.5 flex-1 ${step >= s.id ? "opacity-100" : "opacity-30"}`}>
                        <div className={`w-1.5 h-1.5 rounded-full transition-colors ${step > s.id ? "bg-primary/60" : step === s.id ? "bg-accent" : "bg-border"}`} />
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        ...
        >
            ...
            <div className="w-full max-w-xs space-y-3">
                <motion.button
                    whileTap={{ scale: 0.97 }} onClick={() => navigate("/verification")}
                    className="w-full py-4 rounded-2xl text-primary-foreground font-semibold shadow-lg"
                    style={{ background: "var(--gradient-primary)" }}
                >
                    View Matches
                </motion.button>
                <button onClick={() => navigate("/profile")} className="w-full py-3 text-sm text-muted-foreground underline underline-offset-2">
                    Edit Profile
                </button>
            </div>
        </motion.div>
...
function AutoLocationCard({ form, setForm }) {
    ...
    useEffect(() => {
        if (!requestedRef.current && !form.latitude && !form.longitude) {
    ...
    return (
        <div className="rounded-xl border border-border p-3 bg-muted/40 mt-4">
            <div className="flex items-center justify-between">
                ...
                <button type="button" className="text-xs text-primary underline underline-offset-2" onClick={request}>
                    Get My Location
                </button>
            </div>
            {form.latitude && form.longitude ? (
                <div className="mt-2 text-xs">
                    ...
                    <span className="ml-1 text-muted-foreground">Lat: {form.latitude}, Lng: {form.longitude}</span>
                </div>
            ) : (
                <div className="mt-2 text-xs text-muted-foreground">
                    {locationError ? <span className="text-destructive">{locationError}</span> : "Location not set yet."}
                </div>
            )}
        </div>
    )
    ...
    return (
        <div className="min-h-screen bg-background">
                ...
                <AnimatePresence mode="wait">
                        ...
                        <StepCard key="s1" icon={User} title="Personal Details" subtitle="Basic information about you" variant="primary">
                            ...
                            <div>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                                    {[0, 1, 2, 3].map((idx) => {
                                        const photo = form.photos?.[idx] || null
                                                    ...
                                                    <>
                                                        <img
                                                            ...
                                                            style={{
                                                                ...
                                                                objectFit: "cover",
                                                                display: "block",
                                                                filter: form.photos_blurred ? "blur(3px)" : "none",  // ← blur
                                                        ...
                                                        <div style={{
                                                            ...
                                                            <Camera style={{ width: 12, height: 12, color: "#555" }} />
                                                        </div>
                                                    </>
                                                ) : (
                                                    <>
                                                        <Camera style={{ width: 22, height: 22, color: "#bbb" }} />
                                                        <span style={{ fontSize: 9, color: "#bbb" }}>
                                                            {isFirst ? "Main Photo" : `Photo ${idx + 1}`}
                                                        </span>
                                                    </>
                                                )}
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10 }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                        <span style={{ fontSize: 11, color: "var(--muted-foreground)" }}>
                                            {form.photos_blurred ? "Photos blurred" : "Photos visible"}
                                        </span>
                                        <div
                                            onClick={() => set("photos_blurred")(!form.photos_blurred)}
                                            style={{
                                                width: 40, height: 22, borderRadius: 99,
                                                background: form.photos_blurred ? "var(--primary)" : "#ccc",
                            ...
                            <RSelect label="Marital Status" value={form.marital_status} onChange={set("marital_status")} placeholder="Select marital status" options={MARITAL_STATUSES} />
                        </StepCard>
                    )}
                    ...
                    {step === 2 && (
                        <StepCard key="s2" icon={MapPin} title="Location & Contact" subtitle="Where are you based?" variant="muted">
                            ...
                            <ToggleGroup
                                label="Hide Contact from Matches?" value={form.contact_hidden} onChange={set("contact_hidden")}
                                options={[
                    ...
                    {step === 6 && (
                        <StepCard key="s6" icon={Sparkles} title="Lifestyle & About You" subtitle="Let matches know who you are" variant="primary">
                            ...
                            <ToggleGroup
                                label="Willing to Relocate?" value={form.willing_to_relocate} onChange={set("willing_to_relocate")} optional
                                options={[
                    ...
                    {step === 8 && (
                        <motion.div key="s8" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-4">
                            ...
                            <StepCard icon={User} title="Demographics" subtitle="Age, gender, marital status" variant="primary">
                                <ToggleGroup
                                    label="Preferred Gender" value={prefs.pref_gender} onChange={setp("pref_gender")} optional
                                    options={[
                            ...
                            <StepCard icon={Sparkles} title="Life Situation" subtitle="Children & relocation" variant="primary">
                                ...
                                <ToggleGroup
                                    label="Willing to Relocate" value={prefs.pref_willing_to_relocate} onChange={setp("pref_willing_to_relocate")} optional
                                    options={[
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border px-4 py-4 space-y-2">
                ...
                {step === TOTAL_STEPS && (
                    <button
                        onClick={() => ctrl.submit(form, prefs)}
                        disabled={saving}
