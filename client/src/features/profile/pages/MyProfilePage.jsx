    ...
    return (
        <div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 8 }}>
                {interests.map((item, i) => (
                    <span key={i} style={{
                        ...
                        backgroundColor: "var(--color-secondary)", fontSize: 15, color: "var(--color-primary)",
                    }}>
                        {item}
            ...
            <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={add} style={{ padding: "6px 14px", borderRadius: 12, background: "var(--color-primary)", color: "var(--color-primary-foreground)", fontSize: 12, border: "none" }}>
                    Add
                </button>
            </div>
        </div>
...
const inputStyle = {
    ...
    outline: "none", color: "#333", background: "#fff", boxSizing: "border-box",
};
        ...
        <div style={{ height: "100%", overflowY: "auto", backgroundColor: "#f0f0f0", padding: 10 }}>
            ...
            <div style={{ maxWidth: 640, margin: "0 auto", paddingBottom: 60 }}>
                ...
                <div style={{
                    ...
                    backgroundColor: "#fff", padding: "16px", borderBottom: "1px solid #eee",
                        ...
                        {editMode ? (
                            <>
                                <motion.button whileTap={{ scale: 0.95 }} onClick={handleCancel}
                                    style={{ height: 36, padding: "0 16px", borderRadius: 20, border: "1px solid #e5e7eb", fontSize: 13, display: "flex", alignItems: "center", gap: 4, color: "#555", background: "#fff" }}>
                                    <X style={{ width: 14, height: 14 }} /> Cancel
                                </motion.button>
                                <motion.button whileTap={{ scale: 0.95 }} onClick={handleSave} disabled={saving}
                                    style={{ height: 36, padding: "0 16px", borderRadius: 20, background: "var(--color-primary)", color: "var(--color-primary-foreground)", fontSize: 13, display: "flex", alignItems: "center", gap: 4, border: "none" }}>
                                    {saving ? <Loader2 style={{ width: 14, height: 14 }} className="animate-spin" /> : <Check style={{ width: 14, height: 14 }} />} Save
                                </motion.button>
                            </>
                        ) : (
                            <motion.button whileTap={{ scale: 0.95 }} onClick={() => setEditMode(true)}
                                style={{ width: 40, height: 40, borderRadius: "50%", backgroundColor: "var(--color-primary)", display: "flex", justifyContent: "center", alignItems: "center", border: "none" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                        {photos.map((photo, idx) => (
                            <motion.div key={idx}
                                ...
                                style={{ flex: "1 1 0", borderRadius: 20, overflow: "hidden", position: "relative", aspectRatio: "3/4", backgroundColor: "#eee", cursor: "pointer", direction: "rtl" }}>
                                <img src={photo} alt={`Photo ${idx + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="eager" />
                                {uploadingIdx === idx && (
                                    <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                        <Loader2 style={{ width: 24, height: 24, color: "#fff" }} className="animate-spin" />
                                    </div>
                                )}
                                {idx === 0 && <div style={{ position: "absolute", top: 8, right: 8, padding: "2px 6px", borderRadius: 8, background: "var(--color-primary)", color: "var(--color-primary-foreground)", fontSize: 10 }}>Main</div>}
                                <motion.button whileTap={{ scale: 0.9 }}
                                    style={{ position: "absolute", bottom: 8, left: 8, width: 32, height: 32, borderRadius: "50%", backgroundColor: "#fff", display: "flex", justifyContent: "center", alignItems: "center", border: "none" }}>
                        ...
                        {photos.length < 4 && (
                            <motion.button whileTap={{ scale: 0.97 }} onClick={() => handlePhotoClick(photos.length)}
                                style={{ flex: "1 1 0", borderRadius: 20, border: "2px dashed var(--color-primary)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, aspectRatio: "3/4", background: "#fafafa", cursor: "pointer" }}>
                                {uploadingIdx === photos.length
                                    ? <Loader2 style={{ width: 28, height: 28, color: "var(--color-primary)" }} className="animate-spin" />
                                    : <><Camera style={{ width: 32, height: 32, color: "#888" }} /><span style={{ fontSize: 10, color: "#888" }}>Add Photo</span></>}
                            </motion.button>
                        )}
                    </div>
                </div>
                        ...
                        {editMode ? (
                            <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                                <input value={form.name} onChange={(e) => setField("name", e.target.value)} placeholder="Name"
                                    style={{ flex: 1, padding: "6px 12px", borderRadius: 12, border: "1.5px solid var(--color-primary)", fontSize: 20, outline: "none", color: "#333", background: "#fff" }} />
                                <input value={form.age} onChange={(e) => setField("age", e.target.value)} placeholder="Age" type="number"
                                    style={{ width: 70, padding: "6px 10px", borderRadius: 12, border: "1.5px solid var(--color-primary)", fontSize: 20, outline: "none", color: "#333", background: "#fff" }} />
                            </div>
                        ) : (
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            {editMode
                                ? <input value={form.profession} onChange={(e) => setField("profession", e.target.value)} placeholder="Profession"
                                    style={{ padding: "4px 10px", borderRadius: 12, border: "1.5px solid var(--color-primary)", fontSize: 13, outline: "none", color: "#333", background: "#fff", width: 160 }} />
                ...
                <div style={{ backgroundColor: "#fff", padding: 24, marginBottom: 12, borderRadius: 16 }}>
                    ...
                    {editMode ? (
                        <textarea value={form.bio} onChange={(e) => setField("bio", e.target.value)} rows={4} placeholder="Tell others about yourself..."
                            style={{ width: "100%", padding: "10px 12px", borderRadius: 12, border: "1.5px solid var(--color-primary)", fontSize: 13, lineHeight: 1.6, resize: "none", outline: "none", color: "#333", background: "#fff", boxSizing: "border-box" }} />
                    ) : (
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                            {interests.length > 0
                                ? interests.map((item, idx) => (
                                    <Badge key={idx} variant="accent" active={item} style={{ fontSize: 14, padding: "6px 16px" }}>
                                        {item}
                                    </Badge>
                                ))
                ...
                <div style={{ backgroundColor: "#fff", padding: 24, marginBottom: 12, borderRadius: 16 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <div style={{ width: 32, height: 32, borderRadius: "50%", backgroundColor: "#ede9fe", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        {hasGuardian && !editGuardian && (
                            <motion.button whileTap={{ scale: 0.95 }} onClick={() => setEditGuardian(true)}
                                style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 14px", borderRadius: 16, backgroundColor: "var(--color-primary)", border: "none", cursor: "pointer" }}>
                        ...
                        {editGuardian && (
                            <div style={{ display: "flex", gap: 6 }}>
                                <motion.button whileTap={{ scale: 0.95 }} onClick={handleCancelGuardian}
                                    style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 16, border: "1px solid #e5e7eb", cursor: "pointer", fontSize: 12, color: "#555", background: "#fff" }}>
                                    <X style={{ width: 13, height: 13 }} /> Cancel
                                </motion.button>
                                <motion.button whileTap={{ scale: 0.95 }} onClick={handleSaveGuardian} disabled={savingGuardian}
                                    style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 14px", borderRadius: 16, background: "var(--color-primary)", color: "var(--color-primary-foreground)", border: "none", cursor: "pointer", fontSize: 12 }}>
                                    {savingGuardian ? <Loader2 style={{ width: 13, height: 13 }} className="animate-spin" /> : <Check style={{ width: 13, height: 13 }} />} Save
                                </motion.button>
                            </div>
                        )}
                    </div>
                    ...
                    {/* Privacy note */}
                    <div style={{ backgroundColor: "#f5f3ff", border: "1px solid #ede9fe", borderRadius: 12, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#6d28d9", lineHeight: 1.5 }}>
                    ...
                    {editGuardian ? (
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                            {[
                                ...
                                <div key={key}>
                                    <div style={{ fontSize: 11, color: "#aaa", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>{label}</div>
                                    <input value={guardianForm[key]} onChange={(e) => setGField(key, e.target.value)}
                                        placeholder={placeholder} style={{ ...inputStyle, boxSizing: "border-box" }} />
                                </div>
                            ))}
                        </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            {[
                                ...
                                <div key={sub} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                    <div style={{ width: 36, height: 36, borderRadius: "50%", backgroundColor: bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        ...
                        <div style={{ textAlign: "center", padding: "20px 0" }}>
                            ...
                            <motion.button whileTap={{ scale: 0.95 }}
                                onClick={() => navigate("/add-guardian")}
                                style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 22px", borderRadius: 16, background: "var(--color-primary)", color: "var(--color-primary-foreground)", fontSize: 13, border: "none", cursor: "pointer" }}>
                    ...
                    <div style={{ backgroundColor: "#fff", marginBottom: 12, borderRadius: 16 }}>
                        {[
                            ...
                            <motion.button key={label} whileTap={{ scale: 0.99 }} onClick={() => navigate(path)}
                                style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 24px", borderBottom: "1px solid #f9fafb", background: "none", textAlign: "left", borderRadius: 0 }}>
